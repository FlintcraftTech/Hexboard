...
- **The key inventory is governed by a canonical manifest.** `resources/key-layout.json` is the single source of truth for which characters the keyboard offers and where they sit; `resources/key-manifest.md` is a human-readable view generated from it, and is not hand-edited. The config carries the key inventory only — geometry stays in code, so a variant may change which characters exist but never the perceptual geometry. **The manifest governs the three letter and symbol panels and nothing else.** The emoji panels are system-supplied content rather than a hand-curated inventory, so they are outside its scope: the four rules below do not apply to them, and their absence from the config is correct rather than a gap. Four rules hold: no key may be lost (verify the shipped key set against the config before shipping); no unresolved duplicates (a character lives on one panel unless a second copy is deliberately justified); no silent changes (any move, addition, or removal is reflected in the config and noted in the commit); and empty slots are opportunities, not acceptable gaps (a freed slot gets filled with a character that has no other home, agreed first).
- **Predictive text is planned but deferred, and will be built rather than borrowed.** The eventual version won't fail every time a typo has the wrong first letter. The layout is what makes this fixable: in a hexagonal tessellation each interior key has exactly six equidistant neighbours, so a pressed key implies a small, uniform set of keys the user might have meant. The engine searches forwards from every plausible first letter rather than trusting the one that was pressed, ranking candidates by geometric plausibility and word frequency; searching the letters backwards stays useful for typos later in a word. No general-purpose library can do this, because none of them know the key geometry — that is the reason to build the engine rather than adopt one. The neighbour sets are derived rather than hand-maintained, from `resources/key-layout.json` together with the zag rule that lives in code — the config gives each key's row and column, and the zag parity determines which six keys actually neighbour it. So the neighbour table is computed at runtime from both, not stored in the config. Held until after the first working model — get the keyboard right first.
