...
#### [freeform] Wrap the flat checkout into the nested shape, product inside, planning documents outside [nested-wrap]
Runs alone
This folder becomes a private outer repository holding the method's documents, and the existing public repository becomes an inner one holding only the product. Chosen by you on 2026-09-17 during a setup top-up, which offers the conversion to every flat project.

**It reverses a decision, knowingly, and that is recorded rather than glossed.** On 2026-08-06 this project chose to publish its planning record — `LOG/`, `QUEUE.md`, `CLAUDE.md` and `SPEC.md` tracked and public — because the record is a worked example of the Throughliner method and the demonstration value was the point. The nested shape makes exactly those documents private. Claude recommended declining on that ground and you chose the conversion anyway, so the older decision is superseded rather than forgotten: if the planning record is ever to be published again it is a deliberate move from here, not a return to a default.
...
- Coming back up to the private outer: `SPEC.md`, `QUEUE.md`, `CLAUDE.md`, `CYCLES.md`, `TOOLS.md`, `LOG/`, `FAQ/`, `INBOX/`, `workshop/`, `temp/`, `.throughliner-version`, `.throughliner-format-epoch`, `.throughliner/`, and `.claude/`.
- Also outer, decided by you when both were put as borderline: `hexboard17.html`, the frozen browser prototype, and `planning/`, which holds the layout-preview fixture. Neither ships; both are reference material the method's documents point at.

**The product subfolder is named `hexboard/`**, the project's own name for the thing being built.

**Every file and folder moves down by explicit name, `.git` included — never a glob and never a loop.** That is the method's own instruction for this conversion and it is the one step where a mistake is expensive: a glob that catches `.` or misses `.git` leaves a repository whose worktree root has moved out from under it.

**The part that is most of the work: about twenty queue items name paths that all move one level deeper.** Items whose Files lines read `android/app/src/...`, `resources/...` or `scripts/...` become wrong the moment the move lands, and a build reading a stale path either fails or is refused by the safety check. The rewrite is mechanical rather than a judgment, but it has to land in the same move as the files — a checkout in the new shape with a queue still describing the old one is the failure state this item exists to avoid. `CLAUDE.md`'s own references to `planning/layout-preview.html` and `hexboard17.html` are unaffected, both staying at the top.

**Two more ripples to check as part of the move rather than after it.** `android/local.properties` carries the relocated build-output path, which is absolute and off this folder, so it should survive — but it is read at the new depth and wants confirming rather than assuming. And Android Studio holds the project's own path; it will need reopening at `hexboard/`.

**Visibility and the project rules both change with the shape.** `CLAUDE.md`'s Visibility line currently says this folder's repository holds the documents and they go public; after the wrap it names both repositories and their roles. The project-rules paragraph stating that the repo is going public and the planning record goes with it is no longer true as written and is rewritten in the same move.

**Why freeform and why it runs alone.** It cannot run inside a build: it moves the paths every other item names, including the queue file's own referents and the repository root the run is executing in, so running the mechanism to perform it is the failure. `Runs alone` is written as well because the marker is what stops a run reaching it alongside other work; the two are not alternatives here.

**It needs a clean working tree, which is why it is not done in the session that planned it.** The outer's documents are moved *out* of the inner repository, so the inner sees them as deletions of their last committed state — and any uncommitted edit to them at that moment is separated from the history that explains it. Committing first makes the wrap a pure restructure.
...
**Options already refused.** Leaving the flat shape — recommended by Claude on the 2026-08-06 decision and declined by you. The split arm — it creates a new inner repository, which is wrong where one already exists with the remote on it. Doing the path rewrite as a follow-up item — it leaves a window in which every cleared build reads paths that no longer exist, and nothing would stop a run entering it.
...
**Commonness bounds what a correction may produce, settled with you on 2026-09-17.** Every word in the shipped list is recognised and so is never corrected — that is the first guard below, unchanged. What changes is the other direction: the engine may only ever *produce* a word from the commoner half of the list. A mis-tap one neighbour away from an obscure word is left alone rather than turned into it.

**Which bands, as a proportion rather than a figure.** `resources/wordlist-en.txt` is SCOWL at level 60 and below, about 104,000 words, each line carrying its level; the levels present are 10, 20, 35, 40, 50, 55 and 60. Taking bands from the commonest upward until half the list is covered reaches level 35 and stops, at about 51,000 words. The rule is therefore "the commonest bands that together make up no more than half the list", which re-derives itself if the generation level changes, rather than a hardcoded 35. Read from the file on 2026-09-17.

**Within a band there is no ordering at all, which is the honest limit of this.** One band holds 38,000 words, so "the commonest candidate" often resolves to a band rather than a word. The engine then takes the shorter word, shorter being likelier in general — a stand-in for frequency data rather than a ranking, and named as such in the code. [predictive-frequency-table] is what would replace it.

**Why a ceiling rather than the tie-break this item carried before.** A tie-break only speaks when two candidates are exactly equally close, which is rare, so the engine was in practice free to correct toward anything in the list. A ceiling speaks on every correction. It also broke the trade [word-list-size-level] was stuck on — that entry could not raise the list's generation level to catch missing everyday words, because a higher level admitted obscure words the engine could then correct toward, and separating what is recognised from what may be produced removes the coupling. That entry was deleted on 2026-09-17 once SCOWL's own README settled the level question outright; the reasoning is in `workshop/resources/research/word-list-licence-and-frequency.md` and the by-name route survives on [word-list-ok-by-name]. No second frequency source is bundled, as [predictive-dictionary-bundle] settled on 2026-09-04, and [predictive-frequency-table] carries what would replace the bands with a real ranking — held until this item has shipped, since whether the bands read acceptably can only be seen by typing on the engine. That ordering is written on both entries.

**What the build changes.**
- `android/app/src/main/java/tech/flintcraft/hexboard/Autocorrect.kt` — new. The neighbour-weighted edit distance between a typed word and a candidate, and the choice of closest match from the eligible candidates only — those in the commonest bands that together make up no more than half the list, that bound derived from the list's own level counts rather than written as a number, with the shorter word winning within a band. Pure functions over a word, a neighbour table and the word list, so they can be checked without a running input method.
...
**The observation that shows it landed:** `AutocorrectTest` passes, asserting that a word one neighbour-substitution from a dictionary word is corrected to it, that a word one non-neighbour substitution away is left alone, that a word already in the list is never changed however rare its band, that no correction ever produces a word outside the eligible bands even where an ineligible one is strictly nearer, and that the eligible set is derived from the list's own level counts rather than from a literal 35. `AutocorrectUiTest` types a near-miss followed by a space and asserts the corrected word is in the field, then presses backspace and asserts the original is back. Nothing on this machine can run either, so running them is Android Studio's, on the Pixel 6.

**Options already refused, each with what defeated it.** Correcting per keystroke, searching forwards from the pressed key and its six neighbours and ranking by plausibility times frequency — this was the 2026-08-06 design and it lost to word-boundary correction on 2026-08-20, because it exists to work around reading a word before it is finished, which correcting at the boundary simply removes; it was also far more machinery, needing a per-keystroke candidate interface. Adopting an existing library — none knows the key geometry, which is the entire source of the advantage. Learning from what the user types — designed out on 2026-08-20 and now [predictive-saved-words]' business rather than this item's. Correcting grammar or punctuation — barred by SPEC. Prefix lookup over commonness tiers, cascading as each tier runs out of matches, which the user brought on 2026-09-17 from a specification written in a session with another assistant — refused because a prefix search treats the first letter as certain, which is the very complaint word-boundary correction was chosen to dissolve on 2026-08-20, and because it cannot use the six-neighbour geometry that is this engine's whole advantage: type `vha` for `cha` and every tier returns nothing. Commonness as a tie-break alone — the version this item carried until 2026-09-17, defeated above.
...
Rests on: the neighbour table and the word list, both built by this item's two blockers rather than assumed; SPEC's three guards, read from `SPEC.md` on 2026-09-04; that this project's own predictive-text deferral has been met, established by the install of 2026-09-03 recorded in [install-and-enable-on-pixel]; the shipped list's seven size levels and their word counts, and that every line carries its level, read from `resources/wordlist-en.txt` on 2026-09-17 — the bound is computed from those counts at build time rather than trusted as a figure; that a level is a commonness ranking at all, which is SCOWL's own account of how it assigned them, recorded in the research file above and not independently checked.

[completion-slots] follows this item in the cleared region: its left slot exists to protect a word from a correction, so it means nothing until this engine does. That ordering is written on both entries. [predictive-saved-words] is held against this item, being the store the engine would consult once it exists. [tap-word-alternatives] is a different mechanism and this engine cannot reach it: it would never touch "rose" for "rows", both being real words several keys apart. Those orderings are written on all three entries.

#### Three slots above the keys: what you typed, and the two commonest completions [completion-slots]
Red flag · State: cleared
While a word is being typed, the row above the keys shows three things — the exact string entered so far, and the two commonest words that could complete it, commonest first. Tapping any of them puts that word into the text. Nothing is applied unless it is tapped.

Designed with you on 2026-09-17, from a specification you brought from a session with another assistant. Your own want, in your framing: not to see words suggested in random order, but to see commoner ones preferred.

**Why this is an offer and never a correction.** Completion is prefix-directed by nature — it reads a word forwards and treats the first letter as certain — which is exactly the mechanism [uniform-neighbours-predictive] refused for correcting. SPEC already draws the line these two sit either side of: offering an alternative the user can tap is permitted, applying one is not. So a completion that misses because the first letter was mistyped simply shows nothing, and the space bar's geometry-based correction still catches the word. The two stop competing once one of them is barred from acting on its own.

**What each slot is.** The left slot carries the raw string, untouched, and tapping it commits that string and a space with no correction fired — it protects the word this once and does not save it. Saving stays the deliberate act [predictive-saved-words] settled on 2026-09-12, reached by tapping the offer that appears when a correction is undone. A slot tapped in the middle of fast typing would make "deliberate" doubtful, and that store's privacy flag was cleared on the strength of every entry being a considered act. The centre and right slots carry the two commonest completions of the typed string, ordered by the shipped list's own size bands, commonest band first, with the shorter word winning inside a band. Where fewer than two completions exist, the spare slots are blank.
...
**What the build changes.**
- `android/app/src/main/java/tech/flintcraft/hexboard/Completions.kt` — new. Given a typed string, returns the completions from the shipped list ordered by size band and then by length, shortest first. Pure functions over a string and the word list, so they can be checked without a running input method.
- `android/app/src/main/java/tech/flintcraft/hexboard/HexboardImeService.kt` — keeps the word in progress, asks for its completions on each key, publishes the three slots, commits a tapped slot's text with a trailing space and suppresses correction for it, and clears the slots when a space arrives.
- `android/app/src/main/java/tech/flintcraft/hexboard/KeyboardPanel.kt` — the row's middle is divided into three equal slots between the clipboard button at its left end and the microphone at its right, each taking a third of whatever width remains between the two, with a word too long for its slot truncated rather than resized. The slots report which was tapped.
- `android/app/src/test/java/tech/flintcraft/hexboard/CompletionsTest.kt` — new, carrying the first half of the observation.
- `android/app/src/androidTest/java/tech/flintcraft/hexboard/CompletionSlotsUiTest.kt` — new, carrying the second half.

Reads but does not change: `android/app/src/main/java/tech/flintcraft/hexboard/WordList.kt`, for the list and its levels.

**The observation that shows it landed:** `CompletionsTest` passes, asserting that a prefix's completions come back with the commonest band first and the shorter word first within a band, that a prefix with nothing behind it returns an empty list rather than reaching further, and that the raw string is returned untouched whatever the completions are. `CompletionSlotsUiTest` types a prefix, asserts three slots sit between the clipboard button and the microphone, asserts the left one reads exactly what was typed, taps the centre one and asserts the completed word and a space are in the field with nothing corrected. Nothing on this machine can run either, so running them is Android Studio's, on the Pixel 6. Whether three slots are legible at the Pixel 6's key size is a judgment on the phone rather than a test, and it is the one thing here that wants an eye.

**Options already refused, each with what defeated it.** A third fixed role offering a long completion — the "expansion lane" of the specification this came from, refused on 2026-09-17: it answers keystroke savings, which nobody here raised, and it forced a length gate that needed an invented threshold; if typing long words by hand turns out to annoy in real use, that is the complaint that would revive it. Ranking by commonness across all three slots — it leaves no slot showing the raw string, which is the one thing on the row that is not a guess. Commit-and-save on the left slot — recorded above. Tier 0 harvesting contacts, calendar and place names — recorded above. Prefix lookup driving correction rather than offering — refused on [uniform-neighbours-predictive], where the reasoning lives.

Rests on: the row above the keys existing and running, built by [suggestion-strip] on 2026-09-04 and confirmed on the Pixel 6 by the drive of [verify-this-runs-build-on-device] on 2026-09-09; the row's two ends being taken by the clipboard button and the microphone, which are the designs of [persistent-clipboard] and [in-keyboard-voice-input] and are not built, so this item's own three slots are the only thing it can place; the shipped list carrying a size level on every line, read from `resources/wordlist-en.txt` on 2026-09-17; SPEC permitting an offer where it forbids an application, read from `SPEC.md` on 2026-09-17.

Follows [uniform-neighbours-predictive] in the cleared region rather than carrying a blocking line: the left slot's whole job is protecting a word from a correction, so it means nothing until that engine exists, and the two would run in that order anyway. That ordering is written on both entries.
...
[direct-boot-unavailable] waits on this item shipping, alongside [predictive-saved-words]: between them they settle what the keyboard reads at startup, and this store is the case that must *not* move to storage readable before unlock, being encrypted at rest for exactly that reason. That ordering is written on all three entries.
...
**Lifted on 2026-09-09.** [verify-this-runs-build-on-device] was driven to its end that day and the strip this row clamps into is compiled and on the phone. One caveat travelled with that lift: this item's observation is an instrumented test, and the suite was then failing on every test that renders the board. That condition lifted on 2026-09-17, when [instrumented-tests-no-composition] was closed and the suite returned 28 of 28 on the Pixel 6, so the observation below is runnable.
...
**SCOWL's own guidance backs this, read on 2026-09-17 and recorded in `workshop/resources/research/word-list-licence-and-frequency.md`.** Its author recommends size 60 for spell checking as the largest size he is confident carries no misspellings or invalid words, names 70 as the large dictionary, and calls 80 and above unreasonable for spell checking — 80 being the Scrabble words. So raising the level is not a free alternative to adding a word by name, and the by-name route below is the standing answer for anything else the list turns out to lack. [word-list-size-level] asked which level to generate at and was deleted the same day, that question being answered rather than deferred.
...
It surfaced as a test failure rather than as a report from a person: `EmojiPanelsUiTest` asked whether the key's *output* was on screen, found nothing, and fell before its first gesture. That test now asks for the accessibility label instead, which is what the other UI tests already did — so the test is no longer wrong about the code, and this item was the remaining question about whether the code is right.

**Settled with you on 2026-09-17: the announcement tracks the glyph.** A key that types announces what it will actually type in the current shift state; the keys that act keep their spoken names.

**The entry called this a product question, and reading the code narrowed it to almost nothing.** `accessibilityLabel` carries its own doc comment saying *"A letter or punctuation key announces its own glyph, which is what a reader expects"* — and then returns `key.label`, the config's uppercase form, while the drawing two lines above it calls `key.glyph(shiftState)`. The function states the intent and does something else, so this is closer to an unfinished implementation than an undecided design.

**The product argument agrees, and it is the one that decides it.** SPEC makes a point of the board showing which shift state it is in — shift lit differently for the two states, every letter drawn as a capital. A screen-reader user gets none of that, and the label is the only channel where they could. A description that never changes leaves them with no way to know whether the next letter arrives capitalised, which is information every sighted user has.

**Most of the work is the ripple rather than the fix.** Four instrumented tests find keys by their accessibility description, so changing what keys announce changes what those tests must ask for. That is named in the files below so a build meets it in its own scope rather than by being refused.

**What the build changes.**
- `android/app/src/main/java/tech/flintcraft/hexboard/KeyboardPanel.kt` — `accessibilityLabel` takes the shift state and returns `key.glyph(shiftState)` for keys that type, in place of `key.label`; the action keys' spoken names are untouched; its doc comment gains the word that was missing, saying the glyph is the one drawn in the current state.
- `android/app/src/androidTest/java/tech/flintcraft/hexboard/ShiftStateUiTest.kt` — gains the observation below, alongside its existing assertions.
- `android/app/src/androidTest/java/tech/flintcraft/hexboard/EmojiPanelsUiTest.kt`, `KeyConfigUiTest.kt`, `LayoutSwitchingUiTest.kt` — every lookup by accessibility description updated to the form the key now announces. These carry no new assertion; they are here because they break otherwise.

Reads but does not change: `resources/key-layout.json`, to confirm the config's labels are the uppercase forms this reasoning assumes.

**The observation that shows it landed:** `ShiftStateUiTest` passes, asserting that a letter key announces its lowercase form with neither shift nor caps on, its uppercase form with either on, and that a key which acts rather than types announces the same name in every state. The other three instrumented tests pass unchanged in substance. Nothing on this machine can run them, so running them is Android Studio's, on the Pixel 6 — which is available now that the suite returns 28 of 28, per the record under [instrumented-tests-no-composition].

**Options already refused, each with what defeated it.** A case-neutral announcement, the same whatever the shift state — it removes the only channel telling a screen-reader user which state the board is in, which SPEC deliberately shows everyone else. Leaving `key.label` — it disagrees with the drawn key in the state the board spends most of its time in, and contradicts the function's own doc comment. Putting the word "capital" into the label string — how a capital is spoken is the screen reader's business, and a label that says it would double up wherever the reader already does.

Rests on: `accessibilityLabel` returning `key.label`, its doc comment's wording, and `Key.glyph(shiftState)` sitting in the same file, all read from `KeyboardPanel.kt` on 2026-09-17; the four instrumented tests querying by content description, read from `android/app/src/androidTest/` the same day; SPEC's shift-state visibility and its accessibility bullet, read from `SPEC.md` the same day; that a screen reader distinguishes case when speaking a single character, which is platform behaviour reasoned from rather than run — if it does not, the change is neutral rather than wrong, and [verify-a11y-ondevice] is the sitting that would show it.
...
Blocked by: [persistent-clipboard], [predictive-saved-words] until built
...
The trigger for taking it up again is [persistent-clipboard] and [predictive-saved-words] being built, since between them they settle what the keyboard reads at startup. It was named in prose rather than as a blocker because both are already in Processed, and a `Blocked by:` line was then read as resolving the moment an entry was kept — so it would have recorded the ordering without holding anything back.

**Written as the field on 2026-09-17, that reasoning having lapsed.** The line now takes an `until built` suffix, which holds a capture until every named entry has a build record rather than until it is merely agreed — exactly the trigger the paragraph above describes, and the reason it could not be expressed before. So this entry is no longer offered each session; it returns by itself once both have shipped. It is a chain, [predictive-saved-words] being itself held behind [uniform-neighbours-predictive], but a terminating one: that engine is cleared to run with nothing in front of it. That ordering is written on all three entries.
...
**That refusal was answered rather than overturned on 2026-09-17.** Asked whether releasing was something to start on, you said it is still a standing reminder and set the date yourself, two months out. The objection was that nobody could supply a date without guessing at your intentions; you supplied it, so the ground it stood on has gone rather than been argued away. The entry will not be offered again before 2026-11-17, and it returns by itself then with nothing to confirm.

**What goes quiet with it, stated because it is three entries rather than one.** [feedback-funnel-before-store] and [dictionary-asset-packs] are both held against this entry and so were already being passed over; they stay passed over until this one is processed, which now cannot happen before the date. So the whole release cluster is out of view until then. That is the intended effect and not a side one, but it is worth knowing that nothing in that group will surface in the meantime. That ordering is written on all three entries.

**The business-identity question is answered, reported by you on 2026-09-17: the D-U-N-S number was found.** So publishing under a company name rather than a person's has its identifier already, and that is one fewer unknown whenever this is taken up — the number itself is deliberately not written here, this repository being public. [duns-outcome-unknown] was the `[user]` item that went looking for it, filed from the Drive cleanup project's message of 2026-09-14 and deleted on 2026-09-17 once its answer arrived. You added that it is irrelevant until there is something to ship, which is the same reason the date above sits where it does: your stated focus on 2026-09-17 was on shipping a different project first.

**What is genuinely open**, and none of it is a desk decision ahead of that date: whether the first release is public, closed or internal; what an input method has to declare about the data it handles, which for a keyboard is the sensitive question and touches the clipboard store and the microphone directly; how the build is signed and where that key lives; and what the listing says about a keyboard whose differentiator is a perceptual claim.
...
#### Bundle a real word-frequency table, so commonness is a ranking rather than seven buckets [predictive-frequency-table]
Blocked by: [uniform-neighbours-predictive] until built
The shipped list carries SCOWL's size levels and nothing finer. A real frequency table would replace them where the engine needs an order.

Filed on 2026-09-17, when the commonness work gave this a concrete job it did not have before. [uniform-neighbours-predictive] now bounds corrections to the commonest bands and [completion-slots] orders completions by band — and both then hit the same wall: the bands are coarse and wildly uneven, one of them holding about 38,000 words, and *within* a band there is no order at all. Both items fall back on taking the shorter word, which is a stand-in rather than a ranking. This is what would replace it.

**What is already known, from `workshop/resources/research/word-list-licence-and-frequency.md`.** SCOWL carries no frequency data; its levels were assigned from other corpora's frequency classifications, which is why they rank at all. Two tables were found. The `wordfreq` export of 25,000 English words is CC BY-SA 4.0, and ShareAlike raises a real question about a filtered, repackaged table shipped inside an app under a different licence. The Leipzig Corpora Collection lists are reported as CC BY 4.0, which asks only for attribution — the cleaner pick on licence grounds. One trap is recorded there too: `hermitdave/FrequencyWords` presents as MIT, but the MIT licence covers the generator and the data comes from OpenSubtitles under its own terms.

**The licence was read at source on 2026-09-17, which was this entry's first step, and it is done.** The Anubis proof-of-work challenge is still in front of the site and still defeats a plain fetch, but a real browser solves it, and the Terms of Usage page was read through one. Its wording, and the distinction the secondary sources missed: the downloadable text corpora are CC BY, while the project's *other* data is CC BY-NC. So the licence depends on which artifact is taken. The version is still second-hand — the page says "CC BY" with no number where the secondary sources said 4.0. The quotation and the full reading are in `workshop/resources/research/word-list-licence-and-frequency.md`.

**Getting the data is a separate step from reading the terms, and it has not been done.** The terms prohibit automated queries except through the project's own web services, so the route is the download packages rather than a script, and whoever fetches one goes through the same challenge. A browser gets through it, which is how the terms were read — but a download is not a page read, and nobody has tried one. The user noted on 2026-09-17 that this is something he could do himself; that stands as the fallback if the fetch turns out not to be Claude's, and it is a `[user]` item to file at that point rather than now.

**One thing to settle alongside it, noted on the research file as a gap in SPEC rather than in the research.** A frequency table built from subtitles or news reflects the language of that corpus and that decade, so unfamiliar slang can rank above ordinary words. Nobody has said what corpus this project wants its idea of "common" to come from, and choosing a table is choosing that. That choice is deliberately not made here — see the hold below.
