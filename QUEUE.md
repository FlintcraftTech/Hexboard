...
#### Last session advises building key-manifest-rules next [advisory]
By Claude. The last /plan queued key-manifest-rules at the top of Processed, cleared to build — it writes resources/key-manifest.md and a SPEC principle, needs no user input, and nothing unprocessed blocks it. Suggested next step is /next on it. (This advisory is consumed and cleared at the next /plan.)
