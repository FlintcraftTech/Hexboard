...
Current phase: Android implementation is under way and has run on a Pixel 6 — the QWERTY panel draws from the config and types the keys it is aimed at. There is no input method service yet, so it is an app rather than a keyboard. Very little exists, so keep designing before coding rather than rushing new work into the app. Target stack is Kotlin / Jetpack Compose. A working browser prototype (hexboard17.html) demonstrates the layout, gestures, and key inventory and should be treated as the canonical reference for current design intent.
