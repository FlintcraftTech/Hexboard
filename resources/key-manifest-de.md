<!-- GENERATED FILE — DO NOT HAND-EDIT.
     Layout: QWERTZ (German) (qwertz-de), language de.
     Generated from resources/key-layout-de.json by scripts/generate-key-manifest.py.
     Edit the JSON and re-run the script; any edit made here will be overwritten. -->

# Key manifest — QWERTZ (German)

This document is a readable view of one Hexboard layout's key inventory: which characters this layout offers, which panel they live on, and where on that panel they sit. The layout is QWERTZ (German), identified as `qwertz-de`. It is generated from `resources/key-layout-de.json`, which is the actual source of truth. Edit the JSON, then re-run `scripts/generate-key-manifest.py`.
...
The home panel. Eleven columns, because a German phone keyboard shows Ü, Ö, Ä and ß on the letter rows; row 3 carries shift, common punctuation and the two space bars.

- **row0** (cols 0–10): `Q` `W` `E` `R` `T` `Z` `U` `I` `O` `P` `Ü` — All insert keys: Q–P plus Ü.
- **row1** (cols 0–10): `A` `S` `D` `F` `G` `H` `J` `K` `L` `Ö` `Ä` — A–L plus Ö and Ä.
- **row2** (cols 0–10): `⇤` `Y` `X` `C` `V` `B` `N` `M` `ẞ` `⌫` `⇥` — `⇤` cursor-left at col 0; Y–M plus ß at cols 1–8; `⌫` backspace at col 9; `⇥` cursor-right at col 10.
- **row3** (cols 0–10): `⇧` `?` `,` `!` `␣` `'` `␣` `"` `.` `-` `↵` — Full zag row: two space bars, one per thumb, with `↵` enter at col 10 — the eleventh column the wider letter rows opened up.

Non-output keys on this panel: `⇤` cursor-left, `⌫` backspace, `⇥` cursor-right, `⇧` shift, `↵` enter (outputs a newline).
