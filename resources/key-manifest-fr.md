<!-- GENERATED FILE — DO NOT HAND-EDIT.
     Layout: AZERTY (French) (azerty-fr), language fr.
     Generated from resources/key-layout-fr.json by scripts/generate-key-manifest.py.
     Edit the JSON and re-run the script; any edit made here will be overwritten. -->

# Key manifest — AZERTY (French)

This document is a readable view of one Hexboard layout's key inventory: which characters this layout offers, which panel they live on, and where on that panel they sit. The layout is AZERTY (French), identified as `azerty-fr`. It is generated from `resources/key-layout-fr.json`, which is the actual source of truth. Edit the JSON, then re-run `scripts/generate-key-manifest.py`.
...
- **row0** (cols 0–9): `A` `Z` `E` `R` `T` `Y` `U` `I` `O` `P` — All insert keys: A–P.
- **row1** (cols 0–9): `Q` `S` `D` `F` `G` `H` `J` `K` `L` `M` — Q–M. Ten letters, so backspace moves down to row 2.
- **row2** (cols 0–9): `⇤` `W` `X` `C` `V` `B` `N` `⌫` `⇥` `↵` — `⇤` cursor-left at col 0; W–N at cols 1–6; `⌫` backspace at col 7; `⇥` cursor-right at col 8; `↵` enter at col 9.
- **row3** (cols 0–9): `⇧` `?` `,` `!` `␣` `'` `␣` `"` `.` `-` — Full zag row: two space bars, one per thumb.

Non-output keys on this panel: `⇤` cursor-left, `⌫` backspace, `⇥` cursor-right, `↵` enter (outputs a newline), `⇧` shift.
